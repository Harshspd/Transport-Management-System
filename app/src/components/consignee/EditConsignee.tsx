...
import { consigneeSchema } from '@/validations/validationSchema';
...
const EditConsignee: React.FC<EditConsigneeProps> = ({ onSave, onCancel ,selectedId }) => {
  ...
  useEffect(() => {
    const fetchConsignee = async () => {
      if (selectedId) {
        try {
          const res = await getConsigneeById(selectedId);
          const data = res.data;
          setInitialValues({
            name: data.name || '',
  ...
  const handleSubmit = async (values: typeof initialValues, { setSubmitting, resetForm }: any) => {
    try {
      ...
      const res = selectedId ? await updateConsignee(selectedId,payload) : await createConsignee(payload);
      toast.success(`Consignee ${selectedId ? 'updated' : 'created'} successfully!`);
      onSave(res.data);
      resetForm();
    } catch (error) {
