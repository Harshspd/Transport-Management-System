...
const _EditShipment: React.FC<EditShipmentProps> = ({ shipment, onSave, onCancel }) => {
    return (<div>   </div>);
}
  /*  const getDisplayValue = (entity: any) => {
    ...
    const [form, setForm] = useState({
        ...
        expected_delivery_date_time: shipment.expected_delivery_date_and_time? new Date(shipment.expected_delivery_date_and_time).toISOString().slice(0, 16) : '',
    ...
    const [consignerOptions, setConsignerOptions] = useState<Consigner[]>([]);
    const [consigneeOptions, setConsigneeOptions] = useState<Consignee[]>([]);
    ...
    useEffect(() => {
        Promise.all([
            ...
            setConsignerOptions(consigners.map((c: Consigner) => ({ value: c._id, label: c.name })));
            setConsigneeOptions(consignees.map((c: Consignee) => ({ value: c._id, label: c.name })));
            setDriverOptions(drivers.map((d: Driver) => ({ value: d._id, label: d.name })));
            setVehicleOptions(vehicles.map((v: Vehicle) => ({ value: v._id, label: v.vehicle_number })));
        });
    }, []);
        ...
        <form onSubmit={handleSubmit} className="space-y-4 p-2">
            ...
            <div>
                ...
                <input
                    ...
                    value={form.expected_delivery_date_time}
                    onChange={handleChange}
                    className="w-full border border-gray-300 dark:border-gray-600 rounded-lg p-2 text-sm bg-white dark:bg-gray-900 text-gray-800 dark:text-white"
                />
            </div>
