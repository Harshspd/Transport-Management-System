...
const VehicleModal = ({ show, onClose, onAdd }) => {
    ...
    const handleAddNew = async () => {
        ...
        const response = await saveModalEntry('Vehicle', entry);
        if (response.success) {
            onAdd('Vehicle', entry);
            closeModal();
        } else {
    ...
    const fields = [
        ...
        { label: 'Fitness Expiry', name: 'fitnessExpiry', type: 'date' }
    ];
    ...
    const getLeftFields = () => {
        return [
            { label: 'Vehicle Number', name: 'name', type: 'text' },
            { label: 'Vehicle Type', name: 'vehicleType', type: 'text' },
            { label: 'Registration Number', name: 'registrationNumber', type: 'text' }
        ];
    };
        ...
        <div className={`fixed top-17 right-0 w-1/2 h-full bg-white dark:bg-gray-900 shadow-xl z-50 transition-transform duration-500 ease-in-out ${show ? 'translate-x-0' : 'translate-x-full'}`}>
            <div className="p-6">
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-lg font-bold text-gray-800 dark:text-white">Add Vehicle</h2>
